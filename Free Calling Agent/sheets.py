...
_HEADERS = [
    "Timestamp", "Lead Name", "Phone", "Call SID",
    "Outcome", "Quantity", "Occasion", "Budget", "Notes",
]
...
def _log_row(lead_name: str, phone: str, call_sid: str, action: dict):
    ws = _get_sheet()
    outcome = action.get("type", "unknown")
    row = [
...
async def log_call(lead_name: str, phone: str, call_sid: str, action: dict):
    """Async wrapper — runs blocking gspread call in thread pool."""
